...
public interface WordDAO {
    ...
    @Query("SELECT id, simpleWord, fullWord, language  FROM word_table " +
    ...
    @Query("SELECT id, simpleWord, fullWord, language  FROM word_table " +
            "WHERE simpleWord LIKE :prefix AND language = :lang  ORDER BY length(simpleWord) ASC LIMIT 50")
    List<Word> getAllPrefixCandidateLugatMatchesAscOrderedByLength1Prefix(String lang, String prefix);

    @Query("SELECT id, simpleWord, fullWord, language FROM word_table " +
            "WHERE simpleWord LIKE :prefix1 OR simpleWord LIKE :prefix2 " +
            "AND language = :lang  ORDER BY length(simpleWord) DESC LIMIT 50")
